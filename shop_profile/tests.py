...
class ShopProfileExtendedViewsTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(
            email=TEST_EMAIL, password=TEST_PASS, user_type="shop"
    ...
    # def test_basic_update_post_error_handling(self):
    #     """Test basic_update view POST request with database error."""
    #     with patch.object(ShopProfile, 'save', side_effect=DatabaseError("Database error")):
    #         response = self.client.post(reverse("basic_update"), {
    #             "shop_name": "Updated Shop",
    #             "shop_state": "Test District",
    #             "shop_city": "Test Upazilla",
    #             "landmark_1": "Near Station",
    #             "landmark_2": "Near Mall",
    #             "landmark_3": "Near Park",
    #             "status": "true"
    #         })
    #     self.assertEqual(response.status_code, 200)
    #     self.assertTemplateUsed(response, "app/salon_dashboard/update_basic.html")
    #     messages = list(get_messages(response.wsgi_request)) # type: ignore
    #     self.assertEqual(str(messages[0]), "Failed to update shop: Database error")
    #     self.shop.refresh_from_db()
    #     self.assertEqual(self.shop.shop_name, "Test Shop")  # No changes due to error

    # def test_basic_update_post_invalid_file(self):
    #     """Test basic_update view POST request with invalid file upload."""
    ...
    #         "landmark_2": "Near Mall",
    #         "landmark_3": "Near Park",
    #         "shop_picture": invalid_file
    #     })
    #     self.assertEqual(response.status_code, 200)
    #     self.assertTemplateUsed(response, "app/salon_dashboard/update_basic.html")
    #     self.shop.refresh_from_db()
    #     self.assertEqual(self.shop.shop_name, "Updated Shop")
    #     self.assertFalse(self.shop.shop_picture)  # Invalid file should not be saved
    #     self.assertContains(response, "Shop profile updated successfully")

    def test_basic_update_get_empty_district_upazilla(self):
        """Test basic_update view GET request with empty District and Upazilla."""
        District.objects.all().delete()
        Upazilla.objects.all().delete()
        response = self.client.get(reverse("basic_update"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "app/salon_dashboard/update_basic.html")
        self.assertEqual(response.context["user"], self.user)
        self.assertEqual(response.context["shop"], self.shop)
        self.assertEqual(response.context["district"], [])
        self.assertEqual(response.context["Upazilla"], [])

    def test_customers_empty_bookings(self):
        """Test customers view with no bookings."""
        response = self.client.get(reverse("shop_customers"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "app/salon_dashboard/customers.html")
    ...
    def test_customers_multiple_pages(self):
        """Test customers view with multiple pages of bookings."""
        for i in range(15):
            BookingSlot.objects.create(
                user=UserProfile.objects.create(user=User.objects.create_user(
        ...
        response = self.client.get(reverse("shop_customers"), {"page": 2})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "app/salon_dashboard/customers.html")
        bookings = response.context["bookings"]
        self.assertEqual(len(bookings), 5)  # 10 per page, 15 total = 5 on page 2
        self.assertTrue(response.context["page_obj"].has_previous())
        self.assertFalse(response.context["page_obj"].has_next())
        # Verify ordering (-date, -time)
        dates = [b.date for b in bookings]
        self.assertEqual(dates, sorted(dates, reverse=True))

    def test_customers_invalid_page(self):
        """Test customers view with invalid page number."""
        BookingSlot.objects.create(
            user=UserProfile.objects.create(user=User.objects.create_user(
        ...
        response = self.client.get(reverse("shop_customers"), {"page": "invalid"})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "app/salon_dashboard/customers.html")
        bookings = response.context["bookings"]
        self.assertEqual(len(bookings), 1)  # Should default to first page
        self.assertFalse(response.context["page_obj"].has_next())

    def test_customers_non_existent_page(self):
        """Test customers view with non-existent page number."""
        BookingSlot.objects.create(
            user=UserProfile.objects.create(user=User.objects.create_user(
                email="user@example.com", password="password123")),
            shop=self.shop,
        ...
        response = self.client.get(reverse("shop_customers"), {"page": "999"})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "app/salon_dashboard/customers.html")
        bookings = response.context["bookings"]
        self.assertEqual(len(bookings), 1)  # Should return last page (first page)
        self.assertFalse(response.context["page_obj"].has_next())
